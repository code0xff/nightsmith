...

{
  ...
  "manifest": null | {
    ...
    "actions": [
      ...
      { "type": "read", "contractId": string, "function": string, "args"?: value[], "label"?: string }
    ],
    "assertions": [
...
Reads / queries: to LOOK UP a value and print it (no assertion, no state change), use a "read" action — a view/pure function whose result is written to the log console. E.g. "show me usdc's total supply" → { "type":"read", "contractId":"usdc", "function":"totalSupply" }; "what's Bob's USDC balance?" → { "type":"read", "contractId":"usdc", "function":"balanceOf", "args":["Bob"], "label":"Bob's USDC balance" } (a named account in args resolves to its address). For a pure query against an already-running world, use intent "extendWorld" with just the read action(s) appended. The value is printed raw (token decimals are NOT applied) — when you need a decimals-applied balance or a pass/fail check, use a "tokenBalance"/"callResult" assertion instead.
...
Recipients: a mint "to", a transfer "to", and an assertion "account" may be EITHER a named account declared in accounts[] OR a literal 20-byte 0x address. If the user gives a concrete 0x address, use it VERBATIM — never replace it with a named account, and do not add it to accounts[]. A deployContract "deployer" MUST be a named account. A signer — a transfer "from", an approve "owner", a call "from" — is normally a named account, but it MAY also be a literal 0x address, which Nightsmith impersonates (acts as that address without its key — for "as the owner 0x…" / fork-whale scenarios). Prefer a named account unless the user specifically wants to act as a given address.

Custom contracts: to deploy a user-uploaded contract, add a contract { "id": "...", "kind": "artifact", "name": "<exact uploaded name>" } — do NOT include abi/bytecode (the server fills them by name). Deploy with a deployContract action carrying constructor "args". Call functions via { "type":"call", "contractId", "function", "args", "from" (named account), "value"? } and verify with { "type":"callResult", "contractId", "function", "args", "expected" }. Arg values: integers as decimal strings, an address arg (for call/callResult/read) as a named account OR a 0x string — a named account resolves to its address, so prefer the account name for a declared account (this covers array-of-address args too); bytes as 0x strings, bool true/false, arrays as JSON arrays, and a tuple/struct as a JSON object keyed by its field names (e.g. {"owner":"0x…","amount":"100"}) or an ordered array. A callResult "expected" is ALWAYS a JSON string, regardless of the function's actual return type — e.g. a bool-returning function is verified with "expected": "true" or "expected": "false" (never a bare JSON boolean), an integer with its decimal string, an address with its 0x string lowercased. Only reference uploaded artifacts listed below.
